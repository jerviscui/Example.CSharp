        ...
        static void RemoveClient(ICacheManager<Student> manager)
        {
            ...
            while (true)
            {
                ...
                if (key.Key == ConsoleKey.D1 || key.Key == ConsoleKey.NumPad1)
                {
                    ...
                    var handle = manager.CacheHandles.First(o => o.Configuration.Name == "memory") as MemoryCacheHandle<Student>;
                    Console.WriteLine(System.Text.Json.JsonSerializer.Serialize(manager.Get("test", "students")));
                    handle!.Remove("test", "students");
                    Console.WriteLine(System.Text.Json.JsonSerializer.Serialize(handle!.Get("test", "students")));
                }
                else if (key.Key == ConsoleKey.D2 || key.Key == ConsoleKey.NumPad2)
                {
                    Console.WriteLine("remove redis");

                    var handle = manager.CacheHandles.First(o => o.Configuration.Name == ConfigKey) as RedisCacheHandle<Student>;
                    handle!.Remove("test", "students");
                }
